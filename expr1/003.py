#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# train_pm25_adagrad_fixed_with_full_visualization.py
...
def predict_test_csv(test_csv_path, model_path, out_csv=PREDICT_CSV):
    ...
    try:
        ...
        df = pd.read_csv(test_csv_path, header=None, encoding='big5', errors='ignore').fillna(0).replace("NR", 0)
...
def main():
    ...
    def metrics(y_t, y_p):
        return mean_squared_error(y_t, y_p), np.sqrt(mean_squared_error(y_t, y_p)), mean_absolute_error(y_t,
                                                                                                        y_p), r2_score(
            y_t, y_p)
    ...
    print(f"Final Bias (Intercept) w[0]: {w[0, 0]:.4f}")
    ...
    # 原始值分布
    plt.subplot(1, 2, 1)
    plt.hist(y_orig.flatten(), bins=50, color='skyblue', edgecolor='black')
    plt.title("Distribution of True PM2.5 (Original Space)")
    plt.xlabel("PM2.5 Value")
    plt.ylabel("Frequency")

    # log1p 空间分布
    plt.subplot(1, 2, 2)
    plt.hist(y_log.flatten(), bins=50, color='lightcoral', edgecolor='black')
    plt.title("Distribution of log(1+PM2.5) (Log Space)")
    plt.xlabel("log(1+PM2.5) Value")
    plt.ylabel("Frequency")
    ...
    max_val = max(np.max(y_val_orig), np.max(y_val_pred))
    min_val = 0.0

    plt.scatter(y_val_orig, y_val_pred, alpha=0.5, s=10)
    plt.plot([min_val, max_val], [min_val, max_val], 'r--', label='Ideal $y=x$')

    plt.xlabel("True PM2.5 (Original Space)")
    plt.ylabel("Predicted PM2.5 (Original Space)")
    plt.title("Validation Set: True vs Predicted PM2.5")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("val_true_vs_pred.png")
    print("Saved validation true vs predicted plot.")

    ## 4. 残差分布直方图 (验证集)
    residuals = y_val_orig - y_val_pred

    plt.figure(figsize=(8, 5))
    plt.hist(residuals.flatten(), bins=50, color='mediumseagreen', edgecolor='black')

    # 绘制均值线
    res_mean = np.mean(residuals)
    plt.axvline(res_mean, color='r', linestyle='dashed', linewidth=1,
                label=f'Mean Residual: {res_mean:.2f}')

    plt.title("Residual Distribution (Validation Set)")
    plt.xlabel("Residuals (True PM2.5 - Predicted PM2.5)")
    plt.ylabel("Frequency")
    plt.legend()
    ...
    ## 5. 特征权重柱状图 (替代'权重变化'曲线)
    feature_weights = w[1:]
    D = feature_weights.shape[0]
    W_matrix = feature_weights.reshape(len(FEATURE_NAMES), 9)

    # 计算每个特征在所有 9 小时上的平均绝对权重作为特征重要性
    mean_abs_weights = np.mean(np.abs(W_matrix), axis=1)

    # 排序
    sorted_idx = np.argsort(mean_abs_weights)[::-1]
    sorted_weights = mean_abs_weights[sorted_idx]
    ...
    plt.title("Feature Importance (Mean Absolute Weight across 9 hours)")
    plt.tight_layout()
    plt.savefig("feature_weights.png")
    print("Saved feature weights plot.")
